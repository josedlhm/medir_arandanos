...
ROOT = Path("/media/andres/ZEDBOX/berry_dataset_enero")
...
def per_sample_prediction(sample_row, save_mask=False):
    ...
    for img_path in sorted(img_dir.glob("*.png")):
        ...
        depth_mm = cv2.bilateralFilter(
            depth_mm,
            d=9,
            sigmaColor=0.05,  # depth similarity (meters)
            sigmaSpace=5     # spatial smoothing (pixels)
        )

        depth_filtered = depth_mm.copy()
        disc_thresh = 0.05  # meters

        dx = np.abs(np.diff(depth_mm, axis=1))
        dy = np.abs(np.diff(depth_mm, axis=0))

        mask = np.zeros_like(depth_filtered, dtype=bool)
        mask[:, :-1] |= dx > disc_thresh
        mask[:-1, :] |= dy > disc_thresh

        depth_mm[mask] = 0.0
        #depth_mm=depth_filtered
        ...
        if len(C) >= 5:
            ...
            # Calculate size
            msa=minor_axis/2
            box_size=major_axis/2; half_side=box_size/2
            cx,cy=center

            x1, y1 = max(0, cx - half_side), max(0, cy - half_side)
            x2, y2 = min(depth_mm.shape[1], cx + half_side), min(depth_mm.shape[0], cy + half_side)

            roi_depth = depth_mm[int(y1):int(y2), int(x1):int(x2)]
            ...
            if valid_depths.size == 0: 
                ...
                Z = np.min(valid_depths)
                fx=1272.44
                fy=1272.67
                focal_length_px=(fx+fy)/2
                d_mm = (minor_axis * Z) / focal_length_px # minor_axis or major_axis
            #print(f"Fruitlet: {d_mm}")
